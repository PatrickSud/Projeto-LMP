...
export function Jornada() {
  ...
  return (
    <div className="p-6 h-full flex flex-col pt-10 pb-28 min-h-screen bg-gray-50 max-w-md mx-auto">
          ...
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            return (
              <div key={dayNum} className="flex flex-col items-center" onClick={() => setSelectedDate(loopDateStr)}>
                <div className={`relative w-9 h-9 rounded-full flex items-center justify-center border-[3px] transition-all cursor-pointer ${isSelected ? 'ring-2 ring-primary ring-offset-2' : 'hover:bg-gray-50'
                  } ${ringClass}`}>
      ...
      <div className={`bg-white rounded-3xl p-6 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] border border-gray-100 transition-all duration-300 ${selectedInfo ? 'ring-1 ring-primary-light/50' : ''
        }`}>
        ...
        {!isSelectedFuture && selectedDayFinances.length > 0 && (
          <div className="mt-4 pt-4 border-t border-gray-100">
            ...
            <div className="flex gap-2 mb-3">
              ...
              <div className={`flex-1 min-w-0 rounded-xl p-2.5 border ${selectedDayBalance >= 0 ? 'bg-primary-lilac/20 border-primary-lilac/40' : 'bg-orange-50 border-orange-100'
                }`}>
                <Wallet size={11} className={`mb-1 ${selectedDayBalance >= 0 ? 'text-primary-dark' : 'text-orange-500'}`} />
                <p className={`text-[11px] font-bold truncate ${selectedDayBalance >= 0 ? 'text-primary-dark' : 'text-orange-600'
                  }`}>
            ...
            <div className="space-y-2">
              {selectedDayFinances.map((r) => (
                <div key={r.id} className="flex items-center gap-2 text-xs">
                  <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${r.type === 'income' ? 'bg-emerald-400' : 'bg-rose-400'
                    }`} />
                  ...
                  <span className={`font-bold flex-shrink-0 ${r.type === 'income' ? 'text-emerald-600' : 'text-rose-500'
                    }`}>
